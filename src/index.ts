...
import { createCompositionPresentation } from './discord/compositionPresentation.js';
...
const healthState: HealthState = { ready: false, discordUser: null, startedAt: Date.now() };
...
const interactionHandler = new InteractionHandler({ environment, catalog, roleAssignmentService, signupService, stateStore, panelService, logger });

async function resolvePublishChannel(channelId: string) {
...
appServer.setBuildPublisher(async (build, requestedChannelId) => {
  ...
  const { guild, channel } = await resolvePublishChannel(requestedChannelId ?? environment.ROLE_SELECTION_CHANNEL_ID);
...
appServer.setCompositionPublisher(async (composition, channelId) => {
  if (!client.isReady()) throw new Error('El cliente de Discord todavía no está listo.');
  const { guild, channel } = await resolvePublishChannel(channelId);
  const message = await channel.send({ embeds: [createCompositionPresentation(composition)] });
  return { guildId: guild.id, channelId: channel.id, messageId: message.id };
});
...
const syncTimer = setInterval(() => {
  void catalog.refresh().catch((error: unknown) => logger.warn({ err: error }, 'No se pudo refrescar el catálogo desde la API interna.'));
...
client.once(Events.ClientReady, async (readyClient) => {
  ...
  logger.info({ discordUser: readyClient.user.tag, enabledBuilds: catalog.all.length, configVersion: catalog.version }, 'Bot conectado a Discord y sincronizado con la API de builds.');
  ...
  try {
    ...
    if (!channel || !channel.isTextBased()) throw new Error('ROLE_SELECTION_CHANNEL_ID no apunta a un canal de texto válido.');
...
client.on(Events.GuildMemberRemove, (member) => {
  ...
  }).catch((error: unknown) => logger.error({ err: error, userId: member.id }, 'No se pudo liberar el puesto del miembro.'));
});
